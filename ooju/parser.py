...
@dataclass
class ReassignNode:
    """Explicit reassignment via the 'xola' keyword."""
    name: str
    value: str
    line: int
...
class Parser:
    ...
    def parse_statement(self):
        ...
        # xola keyword — explicit reassignment (Phase 3: pedagogical clarity)
        if t.type == TT.KEYWORD and t.value == "xola":
            self.consume()
            name = self.expect(TT.IDENT, "'xola' ৰ পিছত variable name lage").value
            self.expect(TT.ASSIGN, f"'xola {name}' ৰ পিছত '=' lage")
            value = self.collect_until_newline()
            self.skip_newlines()
